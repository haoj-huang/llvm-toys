...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  [&Args, &CmdArgs]() {
  	struct SimpleLogOpts { 
		bool Error = false, Info = false, Debug = false;
		static inline SimpleLogOpts All() { return {true, true, true}; }
		inline operator bool() const { return Error || Info || Debug; }
	} SLG;
	...
	SLG.Error = Args.hasFlag(options::OPT_fuse_error_simple_log, 
	  			  options::OPT_fno_use_error_simple_log, SLG.Error);
	SLG.Info = Args.hasFlag(options::OPT_fuse_info_simple_log, 
	  			  options::OPT_fno_use_info_simple_log, SLG.Info);
	SLG.Debug = Args.hasFlag(options::OPT_fuse_debug_simple_log, 
	  			  options::OPT_fno_use_debug_simple_log, SLG.Debug);
	if (SLG) {
	  CmdArgs.push_back("-include");
